...
from selfhost_client import BaseClient
...
class TestBaseClient(unittest.TestCase):
    ...
    def test_request_auth_header_is_set(self) -> None:
        ...
        self.assertEqual(responses.calls[0].request.headers.get('Authorization'), f'Basic {base64_auth_header}')
    ...
    def test_process_response(self) -> None:
        ...
        with self.subTest('Should raise SelfHostBadRequestException'):
            responses.add(responses.GET, url=self.base_url, status=400)
            response: Response = requests.get(self.base_url)
            self.assertRaises(SelfHostBadRequestException, client._process_response, response)

        with self.subTest('Should raise SelfHostUnauthorizedException'):
            responses.add(responses.GET, url=self.base_url, status=401)
            response: Response = requests.get(self.base_url)
            self.assertRaises(SelfHostUnauthorizedException, client._process_response, response)

        with self.subTest('Should raise SelfHostForbiddenException'):
            responses.add(responses.GET, url=self.base_url, status=403)
            response: Response = requests.get(self.base_url)
            self.assertRaises(SelfHostForbiddenException, client._process_response, response)

        with self.subTest('Should raise SelfHostNotFoundException'):
            responses.add(responses.GET, url=self.base_url, status=404)
            response: Response = requests.get(self.base_url)
            self.assertRaises(SelfHostNotFoundException, client._process_response, response)

        with self.subTest('Should raise SelfHostMethodNotAllowedException'):
            responses.add(responses.GET, url=self.base_url, status=405)
            response: Response = requests.get(self.base_url)
            self.assertRaises(SelfHostMethodNotAllowedException, client._process_response, response)

        with self.subTest('Should raise SelfHostConflictException'):
            responses.add(responses.GET, url=self.base_url, status=409)
            response: Response = requests.get(self.base_url)
            self.assertRaises(SelfHostConflictException, client._process_response, response)

        with self.subTest('Should raise SelfHostTooManyRequestsException'):
            responses.add(responses.GET, url=self.base_url, status=429)
            response: Response = requests.get(self.base_url)
            self.assertRaises(SelfHostTooManyRequestsException, client._process_response, response)

        with self.subTest('Should raise SelfHostInternalServerException'):
            responses.add(responses.GET, url=self.base_url, status=500)
            response: Response = requests.get(self.base_url)
            self.assertRaises(SelfHostInternalServerException, client._process_response, response)
